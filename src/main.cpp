...
{ 
    ...
    for (;;)
    {
        ...
        if(Estado == MOVIENDOSE)
        {
            ...
            switch(caso)
            {
                ...
                case C_ZIGZAG:
                    ...
                    {
                        if(final_ > inicio_) misMotores.giro90(HORARIO);
                        else misMotores.giro90(ANTIHORARIO);
                        //vTaskDelay( xDelay );
                    ...
                    if((contador_lineas_horizontales == lineas_a_contar) && (numero_de_veces_giradas == 1))
                    {
                        if(final_ > inicio_) misMotores.giro90(ANTIHORARIO);
                        else misMotores.giro90(HORARIO);
                        //vTaskDelay( xDelay );
                        contador_lineas_horizontales = 0;
                        numero_de_veces_giradas++;
        ...
        if(Estado == MODO_LABERINTO)
        {
            ...
            vTaskDelay(t2 / portTICK_PERIOD_MS);
            //misMotores.giro90(HORARIO);
            a = misMotores.getVgiro();
            misMotores.setVgiro(a - 200);
            misMotores.giro90(HORARIO);
            misMotores.setVgiro(a);
            misMotores.avanzar();
            vTaskDelay(t1 / portTICK_PERIOD_MS);
            vTaskDelay(t2 / portTICK_PERIOD_MS);
